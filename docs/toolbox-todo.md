...
### TODO-CORE-009：常用输入框增加剪贴板粘贴按钮

- 状态：`[x]`
- 优先级：`P1`
- 类型：体验增强 / 公共能力
- 目标：在常用文本处理工具的输入框上方提供粘贴按钮，点击后读取剪贴板文本并触发页面原有输入刷新逻辑。
- 完成记录：2026-07-23 已扩展 `clipboard-utils.js` 的剪贴板读取和目标输入框粘贴能力，并接入命名转换、正则测试、文本转义、文本拆分、SQL、URL、Base64、哈希摘要和 JWT 共 9 个工具，其中正则测试同时覆盖测试内容和匹配规则；权限失败或剪贴板为空时显示明确提示，各页面同步更新版本说明并补充公共能力和接入测试。
- 进展记录：2026-07-23 已统一检查上述粘贴按钮的横向排列，并增加 `data-clipboard-paste-action` 主操作接线；按钮分别调整为“粘贴并转换/测试/拆分/格式化/编码/计算/解析”，粘贴成功后自动复用页面现有主按钮执行对应操作。
- 进展记录：2026-07-23 已将正则测试“匹配规则”的粘贴按钮移动到输入框正上方并左对齐，同时缩小按钮和文字尺寸。
- 进展记录：2026-07-23 已将“匹配规则”标题与紧凑的“粘贴并测试”按钮合并到同一行左对齐显示。

### TODO-CORE-011：统一版本更新说明弹窗

- 状态：`[x]`
- 优先级：`P1`
- 类型：缺陷修复 / 公共能力
- 目标：统一各工具页面的版本更新弹窗布局、滚动和交互，避免较长更新记录超出视口。
- 完成记录：2026-07-23 已新增公共 `changelog.css`，15 个工具页面统一接入专用版本弹窗类；公共样式负责最大高度、固定标题栏和内容区滚动，公共脚本补充 Esc 关闭、焦点恢复、无障碍属性及打开时回到最新记录。文本拆分页面同步更新至 V1.13。

### TODO-CORE-010：修复自定义导航工具名称纵向换行

- 状态：`[x]`
- 优先级：`P1`
- 类型：缺陷修复 / 响应式布局
- 目标：自定义顶部导航弹窗在系统缩放、浏览器缩放或窄屏下仍完整横向显示工具名称。
- 完成记录：2026-07-23 已隔离弹窗条目中的标签、复选框和名称宽度及换行样式，移除窄屏下条目强制纵向排列，保持“勾选框、完整名称、拖动按钮”单行布局，并补充样式回归测试。
- 进展记录：2026-07-23 已将随机值生成器的标签、输入框和下拉框样式限定在生成配置表单内，避免页面样式继续影响自定义顶部导航弹窗。
...
### TODO-JSON-005：Java DTO toString 格式化

- 状态：`[x]`
- 优先级：`P1`
- 类型：兼容性增强
- 目标：支持将 `ClassName(key=value, ...)` 形式的 Java DTO 字符串修复并格式化为标准 JSON。
- 验收标准：
  - 支持 `null`、数字、中文和普通字符串字段。
  - IP、协议和端口列表值中的逗号不被误识别为字段分隔符。
  - 支持嵌套 DTO。
- 完成记录：2026-07-23 已扩展 `json-java-style-normalizer.js`，支持圆括号 DTO 及嵌套 DTO，字段边界改为识别“逗号 + 下一个 key=”；已覆盖 IP、协议和端口逗号值并将 JSON 格式化工具更新至 V1.93。
...
- 验收标准：
  ...
  - 超长单行仍能兜底拆分。
  - 测试覆盖中文、英文、代码文本。
- 完成记录：2026-07-11 已在文本拆分工具中新增拆分策略选择，支持智能边界、按行、按段落、按句子和硬字符拆分；超长单行、段落或句子会自动回退到兜底拆分，避免超过长度限制；已更新文本拆分工具版本至 V1.06。
...
- 验收标准：
  ...
  - 失败时提示首个差异位置。
  - 模板前后缀可选择是否参与校验。
- 完成记录：2026-07-11 已在文本拆分工具中新增分段合并校验，默认检查原始分段按顺序合并后是否与输入文本一致；可选择校验时包含前缀/后缀模板，校验失败时会提示首个差异字符位置；已更新文本拆分工具版本至 V1.07。

### TODO-SPLIT-005：保护未保存的脱敏配置

- 状态：`[x]`
- 优先级：`P1`
- 类型：交互优化
- 目标：避免用户选择或编辑映射文本时因误点弹窗外部而丢失未保存配置。
- 建议合入：`text-splitter.html`。
- 验收标准：
  - 点击脱敏配置弹窗外部不会关闭弹窗。
  - 按 Esc 键不会关闭脱敏配置弹窗。
  - 仅保存、取消和关闭按钮可以关闭弹窗。
- 完成记录：2026-07-23 已取消脱敏配置弹窗的遮罩点击和 Esc 关闭行为，保留保存、取消、关闭三个明确关闭入口；未保存的编辑内容不会再因误操作丢失；文本拆分工具版本更新至 V1.14。
...
- 验收标准：
  ...
  - 点击后更新倒计时时长。
  - 构建后 `dist` 导航正常。
- 完成记录：2026-07-11 已确认并验收 Neon Timer 倒计时常用预设能力，倒计时空闲状态下支持 5、10、15、25、30、60 分钟预设按钮；已补充集成测试覆盖预设配置、显示条件、点击接线和版本说明，并更新 Neon Timer 版本至 V1.01。
...
- 验收标准：
  ...
  - 声音可开关。
  - 不影响秒表模式。
- 完成记录：2026-07-11 已在 Neon Timer 中新增倒计时结束声音提醒和浏览器通知开关；通知开关会处理不支持、未授权、拒绝和已授权状态，且结束提醒只通过倒计时结束回调触发，不影响秒表模式；已更新 Neon Timer 版本至 V1.02。
...
- 验收标准：
  ...
  - 显示当前阶段。
  - 支持停止和重置。
- 完成记录：2026-07-11 已在 Neon Timer 中新增番茄钟模式，支持工作/休息循环、当前阶段显示、工作和休息时长配置，并复用结束声音与浏览器通知提醒；已更新 Neon Timer 版本至 V1.03。
...
- 验收标准：
  ...
  - 支持键盘开始、暂停、重置。
  - 全屏退出后状态保持。
- 完成记录：2026-07-11 已优化 Neon Timer 全屏展示，新增全屏切换按钮，进入全屏后主计时布局使用完整视口高度，并支持空格开始/暂停、R 重置、F 切换全屏；已更新 Neon Timer 版本至 V1.04。
...
- 完成记录：2026-07-11 已新增哈希/摘要工具 `hash-generator.html`，支持文本和文件的 MD5、SHA-1、SHA-256、SHA-512 摘要计算，支持复制单项/全部结果和摘要比对；已同步导航、首页、README、DEVELOPMENT，并新增 `tests/hash-generator.test.cjs` 覆盖核心计算和工具入口。
...
- 完成记录：2026-07-24 已将字段可视化改为大尺寸弹窗，增加字段流程图和逐位说明两个视图；当前 Cron 的各字段片段、流程节点和详情面板支持点击联动高亮，Cron 工具版本更新至 V1.11。
...
- 完成记录：2026-07-11 曾新增工具配置管理页及公共配置注册表；2026-07-13 已按需求移除该独立工具，顶部导航自定义功能不受影响。
...
- 完成记录：2026-07-11 已新增 `browser-smoke-manifest.js` 和 `tests/browser-smoke.test.cjs`，默认执行静态页面冒烟检查，覆盖共享导航、标题、核心控件标记；设置 `RUN_BROWSER_SMOKE=1` 时可使用 Playwright 执行真实浏览器打开和控件可见性检查。
...
- 完成记录：2026-07-11 已新增 `mobile-layout-baseline.js` 和 `tests/mobile-layout-baseline.test.cjs`，覆盖时间戳、URL、文本转义、文本拆分、正则、SQL、配置管理等重点输入输出页，检查 viewport、移动端断点和关键单列/宽度规则。
