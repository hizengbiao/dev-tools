...
- 进展记录：2026-09-20 已在共享导航内部统一使用 `border-box` 盒模型，避免各工具页面不同的全局盒模型规则改变导航容器宽度及“工具箱首页”的横向位置；静态测试和真实浏览器冒烟测试同步增加导航坐标一致性校验。
- 进展记录：2026-09-20 共享导航展开按钮固定保留四字标签的宽度，“展开全部”与“收起”切换时按钮边界和相邻控件位置保持一致。

## JSON 格式化工具

### TODO-JSON-010：解析与树形编辑逻辑排查

- 状态：`[x]`
- 优先级：`P1`
- 类型：缺陷修复
- 目标：修复格式化修复链路中的内容误改，以及树形编辑、复制和数组删除导致的数据丢失或错位。
- 完成记录：2026-09-07 已修复字符串内部逗号被清理、特殊键名排序/重命名丢失、重命名覆盖已有字段、局部编辑后复制旧值或点击丢失、数组删除后的旧下标、同名 hasOwnProperty 字段导致新增失败及根 JSON 字符串展开/恢复问题；延迟刷新避开正在编辑的节点，版本更新至 V1.99。50 个测试文件通过，真实浏览器交互验证通过，保留 `tests/browser/json-parser-review.js` 供后续复测。
- 完成记录：2026-09-07 第二轮排查修复快速输入、清空和连续树形编辑的撤销/重做，历史快照同时保存展开标记，局部输入框保留原生撤销；修复 DTO 中 URL、引号内类名与单引号日志载荷的错误截取，以及嵌套 JSON 字符串恢复和替换子树后的展开状态；日志标签仅在存在独立载荷时剥离，避免删除数组。版本更新至 V2.00，相关模块增加版本化 URL 以防旧缓存；50 个测试文件和真实浏览器回归通过。
- 完成记录：2026-09-15 已兼容数字开头且包含连字符的单双括号日志标签，并在结构化日志载荷内修复 <code>\_</code>、<code>\@</code> 和字符串中的未转义换行；JSON 格式化工具更新至 V2.01，50 个测试文件和真实浏览器回归通过。
...
### TODO-JSON-006：树形编辑结果状态同步

- 状态：`[x]`
- 优先级：`P1`
- 类型：缺陷修复
- 目标：树形结果编辑或删除键值对后，顶部编辑、压缩和格式化操作必须读取最新 JSON。
- 验收标准：
  - 对象或数组子树编辑后立即点击顶部操作，修改不会丢失。
  - 删除键值对后切换源码或重新格式化，结果保持删除后的状态。
  - 源码模式再次点击“编辑”不会被上一次解析结果覆盖。
- 完成记录：2026-08-13 已将子树编辑的外部点击提交调整到捕获阶段，确保先提交最新值再执行顶部按钮；同步修正格式化、压缩和源码编辑的状态边界，JSON 格式化工具更新至 V1.94，并补充子树编辑后立即编辑、压缩、格式化的回归测试。

### TODO-JSON-007：Java DTO 方括号列表格式化

- 状态：`[x]`
- 优先级：`P1`
- 类型：功能增强
- 目标：支持将 Java DTO 字符串中的协议、IP、地址组、服务、端口等方括号列表转换为标准 JSON 数组。
- 建议合入：`json-parser.html`、`json-java-style-normalizer.js`。
- 验收标准：
  - 未加引号的枚举、协议、IP 和名称转为字符串数组元素。
  - 数字、布尔值、null、空数组和嵌套数组保持正确类型。
  - DTO 中的数组字段不会被日志赋值提取器误当作根 JSON 载荷。
- 完成记录：2026-08-18 已扩展 Java 风格归一化模块，统一处理 DTO 方括号列表的字符串和基本类型，并在日志载荷提取前识别根级 Java DTO/对象，避免只截取第一个数组字段；JSON 格式化工具更新至 V1.95，并覆盖完整 `SecurityStrategyDTO` 样例。
- 完成记录：2026-08-18 已将根级 Java DTO 识别提前到日志标签清理之前，支持外层 DTO、嵌套 DTO、原生 JSON 对象和 DTO 对象列表混合出现，避免把 `relationData={...}` 等内嵌对象误截取为根数据；JSON 格式化工具更新至 V1.96，并覆盖完整 `ExecComputeResDTO` 样例。

### TODO-JSON-008：转义 JSON 格式化卡死防护

- 状态：`[x]`
- 优先级：`P1`
- 类型：缺陷修复
- 目标：格式化带多层反斜杠转义的 JSON 对象或数组时，页面不得因分词游标停滞而卡死。
- 验收标准：
  - 支持有界还原 `\\"` 引号和 `\_` 字段名转义，并输出标准 JSON。
  - 对无法修复的异常输入，扫描逻辑也必须每轮向前推进并快速失败。
- 完成记录：2026-08-31 已增加最多 4 层的转义 JSON 容器解码，并修复未识别转义引号时分词游标不前进导致的无限循环；JSON 格式化工具更新至 V1.97，并补充页面级卡死回归测试。

### TODO-JSON-009：限定类名多层 DTO 格式化

- 状态：`[x]`
- 优先级：`P1`
- 类型：功能增强
- 目标：支持格式化嵌套列表中使用限定类名的 Java DTO，并正确处理 Map 键值里的裸文本转义。
- 验收标准：
  - `OuterDTO.InnerItem(...)` 类型的外层、嵌套 DTO 和 DTO 列表均转为 JSON 对象。
  - 非引号区域的 `\_` 和 `\@` 还原为普通字符，引号内容不被误改。
- 完成记录：2026-08-31 已扩展 Java DTO 类名识别、括号归一化和类名移除逻辑，支持点号分隔的限定类名；新增裸文本 `\_`/`\@` 还原，JSON 格式化工具更新至 V1.98，并覆盖完整多层指标 DTO 样例。
...
- 完成记录：2026-08-13 已限制拆分结果全屏查看的正文最大阅读宽度并居中显示，普通文本及连续长字符均自动换行；全屏内容改为逐逻辑行渲染，使换行后的正文仍与行号准确对应；文本拆分工具版本更新至 V1.15。

### TODO-SPLIT-006：正则表达式脱敏映射

- 状态：`[x]`
- 优先级：`P1`
- 类型：功能增强
- 目标：让脱敏映射支持正则表达式，以便统一脱敏 IP 等具有格式特征的内容。
- 建议合入：`text-splitter.html`、`text-masker.js`。
- 验收标准：
  - 支持固定文本和正则表达式两种映射类型，旧配置无需重新录入。
  - 正则支持常用标志、全局匹配和捕获组替换，并阻止语法无效或会匹配空文本的规则。
  - 正则映射明确为不可逆的单向脱敏，固定文本反脱敏行为保持不变。
  - 提供可直接使用的 IPv4 脱敏示例，并覆盖配置保存、导入导出和窄屏布局。
- 完成记录：2026-08-13 已将脱敏配置升级为固定文本/正则表达式双类型，支持 i/m/s/u 标志、隐式全局匹配和捕获组替换，新增 IPv4 快捷示例及逐行中文校验提示；旧 V1 固定文本配置自动迁移为 V2，正则映射仅参与单向脱敏；文本拆分工具版本更新至 V1.16。
- 完成记录：2026-08-13 已将每段最大长度的默认值由 2900 调整为 2850，文本拆分工具版本更新至 V1.17。
- 完成记录：2026-08-17 修复浏览器缓存旧版 `text-masker.js` 时正则映射被静默降级为固定文本的问题；核心脚本改用版本化 URL，并在运行时版本不匹配时阻止读取、覆写配置和执行脱敏，文本拆分工具版本更新至 V1.18。

### TODO-SPLIT-007：导出配置前选择保存位置

- 状态：`[x]`
- 优先级：`P1`
- 类型：交互优化
- 目标：导出脱敏配置时先选择保存目录和文件名，再写入配置文件。
- 建议合入：`text-splitter.html`。
- 验收标准：
  - 支持文件系统访问 API 的浏览器会在导出前打开系统保存位置选择器。
  - 用户取消选择时不创建文件，也不显示错误提示。
  - 不支持保存选择器的浏览器保留原有下载方式并明确提示。
- 完成记录：2026-09-11 已为脱敏配置导出接入系统保存位置选择器，支持确认目录和文件名后写入、无副作用取消，并保留旧浏览器下载兼容路径；文本拆分工具版本更新至 V1.19。
...
- 完成记录：2026-09-20 SQL 可视化增加独立数据流演示视图，提供可编辑的销售聚合示例及本地单表演算，以来源行号、前后数据表、过滤颜色和箭头展示各阶段变化；支持播放、前后步进及阶段跳转，复杂查询仍可查看结构图。版本更新至 V1.10。

### TODO-SQL-002：SQL 书写与逻辑执行顺序手册

- 完成记录：2026-09-20 SQL 输入区操作栏改为紧凑单行布局，缩小标题和按钮字号、内边距及间距，窄屏支持按钮区横向滚动；版本更新至 V1.11。
- 完成记录：2026-09-20 两侧操作栏统一为 15px 标题、14px 按钮文字及相同按钮高度，保留单行布局并对齐编辑器顶部；版本更新至 V1.12。
- 完成记录：2026-09-20 将执行顺序手册、常用 SQL 示例按钮提至 SQL 工具标题栏，与版本号并列；版本更新至 V1.13。
- 完成记录：2026-09-20 移除 SQL 页旧版版本说明样式，公共弹窗统一条目间距、字号和版本标记对齐，避免页面局部样式叠加导致多余留白；SQL 版本更新至 V1.14。

- 状态：`[x]`
- 优先级：`P2`
- 类型：功能增强 / 使用手册
- 目标：在 SQL 工具内直观对照常用查询关键字的书写顺序与逻辑执行顺序，并结合当前输入标识实际出现的环节。
- 验收标准：
  - 通过页面按钮打开独立悬浮弹窗，不挤占格式化主界面空间。
  - 并列展示 WITH、SELECT、DISTINCT、FROM、JOIN、ON、WHERE、GROUP BY、HAVING、UNION、ORDER BY、LIMIT / OFFSET 的常见书写与逻辑处理顺序。
  - 解释逻辑执行顺序与数据库优化器物理执行计划的区别，并提示通过 EXPLAIN / EXPLAIN ANALYZE 查看实际计划。
  - 根据当前输入 SQL 高亮已识别的关键字环节，并提供别名、过滤、分组和排序等常见误区说明。
- 完成记录：2026-09-20 已新增“执行顺序手册”入口和响应式大尺寸弹窗，提供双栏顺序流程、当前 SQL 环节高亮、关键字速查及优化器说明；SQL 工具版本更新至 V1.07。

### TODO-SQL-003：常用 SQL 示例文档

- 状态：`[x]`
- 优先级：`P2`
- 类型：功能增强 / 使用手册
- 目标：提供脱敏的 MySQL 常用操作示例，支持分类搜索、复制和回填格式化。
- 完成记录：2026-09-20 已新增 11 个示例，覆盖查询、批量插入、INSERT IGNORE、更新、删除、主键、MODIFY / CHANGE、新增列、自增主键及会话变量；修正 CHANGE 能力说明，并补充各操作前提。SQL 工具版本更新至 V1.08。
- 完成记录：2026-09-20 工具统一更名为“SQL 工具”，同步首页、顶部导航、页面标题及相关文档；版本更新至 V1.09。
...
- 优先级：`P1`
- 类型：新增工具
- 目标：将按行输入的关键词与原始内容进行多对多文本匹配，并直观展示双方的匹配状态。
- 建议路径：`keyword-matcher.html`、`keyword-matcher.js`。
- 验收标准：
  - 关键词和原始内容均按行解析，忽略空行、去除行首尾空格并保留原有顺序。
  - 支持包含匹配、完全匹配、区分大小写和可选关键词去重。
  - 关键词与原始内容分别使用绿色和红色展示已匹配、未匹配状态，原始内容高亮具体命中片段。
  - 展示双方总数、已匹配数和未匹配数，并支持复制及 CSV 导出结果。
  - 首页、导航、README、DEVELOPMENT、浏览器冒烟和移动布局基线同步更新。
- 完成记录：2026-09-19 已新增关键词批量匹配工具，完成多对多包含/完全匹配、大小写控制、关键词去重、命中高亮、双侧统计、结果复制及 CSV 导出；核心逻辑覆盖空行、重复项、特殊字符和空关键词回归测试，并同步全部工具入口。
- 完成记录：2026-09-20 已将关键词页面的正文颜色变量改为页面专用命名，避免覆盖共享导航的 `--text-primary`、`--text-secondary`，导航普通文字和当前项高亮恢复为与其他页面一致；工具版本更新至 V1.01。
- 完成记录：2026-09-20 已优化空输入操作：初始进入页面且关键词、原始内容均为空时，点击“开始匹配”会自动载入默认示例并直接生成匹配结果；工具版本更新至 V1.02。
- 完成记录：2026-09-20 已交换页面两列顺序，输入区、统计区和结果区均按“原始内容在左、匹配关键词在右”展示，清空后焦点同步回到左侧原始内容；工具版本更新至 V1.03。
- 完成记录：2026-09-20 已压缩导航下方到标题、标题说明区到输入卡片以及输入卡片顶部的多余留白，编辑器高度保持不变，首屏可用空间增大；展开导航时仍保留公共导航所需顶部间距。工具版本更新至 V1.04。

### JSON 源码行号及错误定位

- 状态：`[x]`
- 完成记录：2026-09-23 两侧输入区在粘贴按钮左侧增加复制按钮，保留原始空格与换行并提供空内容提示；版本更新至 V1.05。
- 完成记录：2026-09-22 修复 FaultChain/FaultPoint 尾随多余单引号干扰嵌套结构扫描的问题，完整样例及转义字符变体均加入回归；版本更新至 V2.06。
- 完成记录：2026-09-22 修复失败新增只读待解析文本查看区，保存与异常对应的实际候选文本，按原始异常坐标高亮并滚动定位；原始输入不变，后续编辑、切换或成功操作清除旧诊断。版本更新至 V2.05。
- 完成记录：2026-09-22 恢复完整英文解析器异常，标注修复中间文本坐标来源；DTO、日志等缺少可靠位置映射时取消首字符误高亮。版本更新至 V2.04。
- 完成记录：2026-09-22 修复键值片段缺少外层括号时的错误定位，虚拟补全后将偏移映射回原文；移除修复中间文本坐标，保证高亮和提示一致，中文冒号提供专门说明。版本更新至 V2.03。
- 完成记录：2026-09-20 JSON 源码编辑增加同步行号、原文错误行列定位、错误行高亮及字符选中；编辑后清除旧标记，支持长文本自动滚动到错误行。版本更新至 V2.02。
...
- 完成记录：2026-07-24 已统一整体执行说明和单字段说明的口径，步进字段会同时展示周期与实际触发位置，例如 `*/30` 显示为每小时第 0、30 分钟；通配字段改为每天、每月、每年等通俗时间语境，不再使用“任意小时”“任意日期”等生硬说法；Cron 工具版本更新至 V1.12。
- 完成记录：2026-07-24 已为包含 `/` 的 Cron 步进字段增加写法拆解，分别解释斜杠左侧的完整范围、起点或限定范围，斜杠的步进取值作用，以及右侧的间隔数值；覆盖 `*/30`、`0/3` 和 `9-18/3` 等写法，Cron 工具版本更新至 V1.13。
- 完成记录：2026-07-24 已将 Cron 字段流程图改为响应式自适应布局，桌面端的节点和连接线会按弹窗可用宽度收缩，使 6/7 段表达式无需横向滚动即可完整显示；窄屏下改用三列布局并隐藏连接线，Cron 工具版本更新至 V1.14。
- 完成记录：2026-07-24 已优化带起点的分钟和秒步进表达式执行说明，触发位置较少时直接列出实际时间点，例如 `6/30 * * * *` 显示为“每小时在第 6、36 分钟执行”；位置较多时使用自然的起点和间隔概括，Cron 工具版本更新至 V1.15。
- 完成记录：2026-07-24 已将多个字段组合步进的兜底说明改为基于实际解析值生成，日期、月份、小时、分钟和秒可组合形成自然的日期范围和执行时刻；例如 `6/30 1/10 2/3 * *` 会列出每月实际日期以及 `01:06` 至 `21:36` 的六个时刻，Cron 工具版本更新至 V1.16。
