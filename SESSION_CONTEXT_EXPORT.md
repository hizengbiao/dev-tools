...
JSON 宽松修复中，根节点为对象或数组且包含反斜杠转义引号时，会先最多解码 4 层并仅在可被 `JSON.parse` 验证时接受结果，同时兼容 `\_` 字段名转义。所有字符扫描分支必须保证游标至少前进一位，避免异常输入卡住主线程。

## 重点工具当前设计

### JSON 格式化

- 修复失败时，异常携带对应的 `parseText`，只读查看区显示该文本并按解析器坐标定位；补全重试失败后仍保留首次异常对应文本，不可用最后一次候选替代。原始输入始终保留。
...
- Java DTO 类名可以是点号分隔的限定名，例如 `OuterDTO.InnerItem(...)`；DTO 裸文本 Map 键值中的 `\_` 和 `\@` 在引号化之前还原，但不改写已在引号内的原生 JSON 内容。
- 树形结果支持层级背景、折叠、编辑、复制路径及 JSON 字符串字段就地展开/恢复。
- 树形编辑或删除后的最新对象必须先于顶部编辑、压缩、格式化操作提交，源码模式重复点击“编辑”不得覆盖用户尚未格式化的手工修改。
- 树形局部操作从当前节点路径读取最新数据；数组删除需重建后续下标并迁移折叠状态。重命名不得覆盖已有字段，排序和路径写入必须保留 `__proto__` 等合法 JSON 键。修复多余逗号时只处理字符串之外的标点。
- 撤销历史记录文本与 JSON 字符串展开路径；每次树形修改和清空保留前后状态，新编辑立即废弃重做分支，局部编辑框使用原生撤销。恢复父 JSON 字符串时先从深到浅恢复已展开后代，替换子树后清理失效路径。
- 根 DTO 在去注释前保护裸 URL；类名前缀移除和赋值载荷边界扫描均区分单双引号，避免修改或截断字符串内容。
- 日志标签只有在边界之外存在独立结构化载荷时才剥离；标签名兼容数字开头及连字符，同时保留纯数字数组。结构化日志载荷中的 `\_`、`\@` 和字符串内未转义换行会在解析前安全归一化。JSON 页通过版本化 URL 加载近期更新的修复、路径编辑、排序及字符串展开模块，修改模块时同步更新对应 URL 标识。
...
- 页面正文颜色使用 `--page-text-primary`、`--page-text-secondary` 专用变量，不得重新定义共享导航依赖的 `--text-primary`、`--text-secondary`，避免深色导航文字被浅色页面变量覆盖。
- 初始进入页面且关键词、原始内容都为空时，点击“开始匹配”会填入默认示例并立即执行；只有一侧为空时仍按用户当前输入正常匹配，不自动覆盖。
- 页面输入区、统计区和结果区统一采用“原始内容在左、匹配关键词在右”的顺序；清空后焦点回到左侧原始内容输入框。
- 页面默认导航收起时采用紧凑顶部间距，标题与输入卡片减少空白但不缩小编辑器；导航展开后必须继续使用公共导航的扩展顶部间距，避免内容被遮挡。

### SQL 工具

- `sql-flow-demo.js` / `sql-flow-ui.js` 提供本地示例数据的逐阶段演算，支持单表查询、比较过滤、分组及常见聚合、HAVING、投影、去重、排序和分页；不模拟真实数据库及隐式类型转换，复杂查询保留结构图。数据流展示输入/输出表格、来源行号、过滤行标记和播放控件，修改 SQL / JSON 后旧结果失效，需重新演算。
...
- MyBatis XML、写操作、DDL、存储过程暂不生成关系图，应返回明确范围提示，原有格式化能力不受影响。
- SQL 页面提供独立“执行顺序手册”弹窗，并列展示常用查询子句的书写顺序与逻辑执行顺序；打开时根据当前输入高亮已识别环节，并明确逻辑顺序不等同于优化器生成的物理执行计划，实际计划需用 EXPLAIN / EXPLAIN ANALYZE 查看。
...
- V1.19 起，导出脱敏配置优先使用系统保存文件选择器，用户确认目录和文件名后才写入；取消选择不生成文件，不支持该 API 的浏览器回退到下载方式并给出提示。
...
- 共享导航及其内部元素统一使用 `border-box`，页面级 `*` 或 `body` 盒模型不得改变导航宽度和横向起点。
