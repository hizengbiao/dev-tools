...
- 支持弹窗式表达式字段可视化，按位置展示字段名称、原始值、可用范围和当前取值含义，并区分 Linux 5 段与 Spring / Quartz 6、7 段；当前表达式片段、字段流程节点和详情支持联动高亮；步进字段同时说明执行间隔和实际触发位置，并拆解斜杠左侧、步进运算符及右侧间隔值；通配字段使用每天、每月、每年等通俗时间语境。
