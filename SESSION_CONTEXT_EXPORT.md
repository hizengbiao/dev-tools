...
- 支持多段 `[...]` 日志标签、赋值前缀、Java toString/Map 风格及 `ClassName(key=value, ...)` DTO 字符串等宽松输入修复；DTO 值中的逗号分隔 IP、协议、端口应完整保留，方括号列表应转换为标准 JSON 数组并区分字符串、数字、布尔值、null、空数组和嵌套数组。
- 日志赋值载荷提取前必须识别根级 Java DTO/Java 对象，避免把 DTO 内第一个 `field=[...]` 数组字段误截取为整个输入。
- 去除 JSON 前日志标签之前也必须先识别根级 Java DTO/Java 对象；DTO 内嵌的原生 `{...}` JSON 对象不能被误当作真正的根 JSON 起点，外层 DTO、嵌套 DTO、原生 JSON 对象和 DTO 对象列表应允许混合解析。
- 树形结果支持层级背景、折叠、编辑、复制路径及 JSON 字符串字段就地展开/恢复。
- 树形编辑或删除后的最新对象必须先于顶部编辑、压缩、格式化操作提交，源码模式重复点击“编辑”不得覆盖用户尚未格式化的手工修改。
...
- 默认按不超过指定字符长度拆分，默认最大长度 2850；优先在换行、标点、空格或词边界拆分。
- 每段可单独复制、复制全部、以 600ms 间隔依次写入剪贴板历史。
- 每段预览约五六行，支持内部滚动和全屏查看；输入和结果支持行号。
- 全屏查看正文限制为 960px 最大阅读宽度并居中显示，按逻辑行渲染行号；普通文本和连续长字符均自动换行，换行后行号仍与对应正文对齐。
- 已移除拆分模式、分段前后缀及模板相关 UI，不要恢复。
- V1.09 新增脱敏/反脱敏；V1.16 将映射扩展为固定文本和正则表达式两种类型，配置格式为 V2，旧 `{plain, masked}` 配置自动迁移为固定文本。正则支持 i/m/s/u 标志、隐式全局匹配及捕获组替换，但属于不可逆的单向脱敏，反脱敏只处理固定文本映射。
- 脱敏逻辑位于 `text-masker.js`，固定文本和正则统一在原文上按“最早命中、同位置最长优先、再按配置顺序”选择候选并单次生成输出，避免映射之间发生连锁替换；保存时校验正则语法、标志和空匹配。
- `text-splitter.html` 通过版本化 URL 加载 V2 脱敏脚本，并在运行时校验 `TextMasker.CONFIG_VERSION`；版本不匹配时不得读取或覆写本地配置，也不得执行脱敏，以免浏览器缓存的 V1 脚本将正则规则静默降级为固定文本。
...
- 版本：V1.16
- 初始提交：`4a2fc5f 增加文本拆分脱敏反脱敏功能`
- 配置存入浏览器，支持 JSON 导入导出，默认勾选启用；V1.16 新增正则映射和 IPv4 示例，旧 V1 配置自动迁移为 V2。
