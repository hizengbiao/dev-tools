...
*   **快捷操作** (鼠标悬停显示):
    ...
    *   **折叠子树**: 输入深度 N，折叠当前节点下深度 N 层及以下的子树。
    *   **删除**: 删除当前节点。
...
11. **安全解析 (V1.74)**: 输入包含函数调用、赋值或其他 JavaScript 表达式的非 JSON 内容，点击“修复”或“格式化”。
    *   **预期**: 不执行输入中的脚本，只按 JSON 修复路径解析；无法修复时显示错误。
12. **空输入状态清理 (V1.75)**: 先格式化任意 JSON，再清空输入框并点击“格式化”。
    *   **预期**: 树视图清空，内部 JSON 对象状态同步清空，复制/压缩不会继续使用旧数据。
13. **排序状态同步 (V1.76)**: 输入 `{"b":2,"a":1}`，格式化后点击“升序”，再点击“格式化”。
    *   **预期**: 结果保持为 `{"a":1,"b":2}` 的升序顺序，不会被旧源码还原。
14. **根节点删除状态清理 (V1.77)**: 格式化任意 JSON 后删除根节点。
    *   **预期**: 输入框、树视图和内部 JSON 对象状态都被清空，不会残留旧 JSON。
15. **折叠子树层级 (V1.78)**: 格式化多层嵌套 JSON，在根节点点击“折叠子树”，输入 `1`。
    *   **预期**: 根节点保持展开，第一层及更深层的对象/数组节点被折叠；输入非整数时显示提示且不改变当前折叠状态。
16. **折叠后真实行号 (V1.79)**: 格式化多层嵌套 JSON 后折叠其中一个子树。
    *   **预期**: 左侧行号仍显示格式化后 JSON 中对应的真实行号，后续可见节点不会按折叠后的可见行重新编号。
...
## 5. 版本更新记录摘要
*   **V1.79**: 折叠树节点后保留格式化 JSON 的真实行号，方便按行号回到源码中定位。
*   **V1.78**: 修复折叠子树层级计算和输入校验，按节点路径稳定折叠指定深度及以下的子树。
*   **V1.77**: 删除根节点时同步清空输入框、树视图和内部 JSON 状态。
*   **V1.76**: 排序后同步源码内容和历史状态，避免再次格式化还原排序。
*   **V1.75**: 清空输入后格式化会同步清空内部 JSON 对象状态。
*   **V1.74**: 修复流程改用安全 JSON 解析，不再动态执行输入内容。
