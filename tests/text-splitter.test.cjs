...
assert.doesNotMatch(page, /\.segment-editor\s*\{[^}]*max-height:/s);
assert.doesNotMatch(page, /\.segment-content\s*\{[^}]*max-height:/s);
assert.match(page, /\.result-list\s*\{[^}]*overflow:\s*visible;/s);
assert.match(page, /\.segment-content\s*\{[^}]*overflow-x:\s*auto;[^}]*overflow-y:\s*hidden;/s);
