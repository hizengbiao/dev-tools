...
assert.match(page, /id="maxLength"[^>]*value="2850"/);
...
assert.match(page, /\.segment-preview-editor\s*\{[^}]*width:\s*min\(960px,\s*100%\);/s);
assert.match(page, /\.segment-preview-line-text\s*\{[^}]*overflow-wrap:\s*anywhere;[^}]*white-space:\s*pre-wrap;[^}]*word-break:\s*break-word;/s);
assert.match(page, /segment\.split\(\/\\r\?\\n\/\)\.forEach/);
assert.match(page, /lineText\.className = 'segment-preview-line-text'/);
...
assert.match(page, /<span>V1\.19<\/span>/);
assert.match(page, /<div class="changelog-date">2026年9月11日<\/div>[\s\S]*?<div class="changelog-version">V1\.19<\/div>/);
...
assert.match(page, /<div class="changelog-date">2026年7月23日<\/div>[\s\S]*?<div class="changelog-version">V1\.13<\/div>/);
...
assert.match(page, /async function exportMaskConfig\(\)/);
assert.match(page, /typeof window\.showSaveFilePicker !== 'function'/);
assert.match(page, /await window\.showSaveFilePicker\(\{/);
assert.match(page, /suggestedName: 'text-splitter-mask-config\.json'/);
assert.match(page, /const writable = await fileHandle\.createWritable\(\)/);
assert.match(page, /await writable\.write\(new Blob\(\[content\]/);
assert.match(page, /await writable\.close\(\)/);
assert.match(page, /error && error\.name === 'AbortError'/);
