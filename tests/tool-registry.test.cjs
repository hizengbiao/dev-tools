...
assert.match(navCss, /\.nav-manager-item\s*\{[^}]*flex-direction:\s*row\s*!important;[^}]*align-items:\s*center\s*!important;/s);
assert.match(navCss, /\.nav-manager-check\s*\{[^}]*flex:\s*1 1 auto;[^}]*white-space:\s*nowrap;/s);
assert.match(navCss, /\.nav-manager-check\s*>\s*span\s*\{[^}]*word-break:\s*keep-all;[^}]*writing-mode:\s*horizontal-tb;/s);
assert.doesNotMatch(navCss, /@media \(max-width:\s*720px\)[\s\S]*?\.nav-manager-item\s*\{[^}]*flex-direction:\s*column;/);
