...
assert.match(page, /<script src="regex-replacement-helper\.js"><\/script>/);
...
assert.match(page, /<div class="changelog-date">2026年7月23日<\/div>[\s\S]*?<div class="changelog-version">V1\.18<\/div>/);
assert.match(page, /class="pattern-paste-header"[\s\S]*?<label for="patternInput">匹配规则<\/label>[\s\S]*?data-clipboard-paste-target="patternInput"/);
assert.match(page, /\.pattern-paste-header\s*\{[\s\S]*?align-items:\s*center;[\s\S]*?justify-content:\s*flex-start;[\s\S]*?gap:\s*8px;/);
assert.match(page, /\.pattern-paste-header \.btn\s*\{[\s\S]*?padding:\s*5px 9px;[\s\S]*?font-size:\s*12px;/);
...
assert.match(page, /data-clipboard-paste-target="patternInput"/);
assert.match(page, /<div class="changelog-date">2026年7月23日<\/div>[\s\S]*?<div class="changelog-version">V1\.15<\/div>/);
...
assert.match(page, /<div class="changelog-date">2026年7月17日<\/div>[\s\S]*?<div class="changelog-version">V1\.13<\/div>/);
assert.match(page, /<div class="changelog-date">2026年7月4日<\/div>/);
...
assert.match(page, /id="replacementGroupHelper"[^>]*aria-live="polite"/);
assert.match(page, /class="replacement-helper"/);
assert.match(page, /function renderReplacementGroupHelper\(matches\)/);
assert.match(page, /RegexReplacementHelper\.buildReplacementGroups\(matches, testText\.value\)/);
assert.match(page, /RegexReplacementHelper\.renderReplacementGroupsText\(matches, testText\.value\)/);
assert.match(page, /没有可用的捕获分组/);
assert.match(page, /可用于替换文本的分组/);
...
const replacementHelperScriptIndex = page.indexOf('<script src="regex-replacement-helper.js"></script>');
...
assert.notEqual(replacementHelperScriptIndex, -1);
...
assert.ok(matchExporterScriptIndex < replacementHelperScriptIndex);
assert.ok(replacementHelperScriptIndex < inlineScriptIndex);
