...
assert.match(page, /class="pattern-paste-actions"[\s\S]*?data-clipboard-paste-target="patternInput"/);
assert.match(page, /\.pattern-paste-actions\s*\{[\s\S]*?justify-content:\s*flex-start;/);
assert.match(page, /\.pattern-paste-actions \.btn\s*\{[\s\S]*?padding:\s*5px 9px;[\s\S]*?font-size:\s*12px;/);
