...
assert.match(page, /<div class="changelog-date">2026年9月20日<\/div>[\s\S]*?<div class="changelog-version">V1\.07<\/div>/);
...
assert.match(page, /id="execution-order-btn"/);
assert.match(page, /id="sql-order-modal"/);
assert.match(page, /SQL 书写顺序与逻辑执行顺序/);
assert.match(page, /EXPLAIN ANALYZE/);
assert.match(page, /function refreshSqlOrderGuide\(\)/);
assert.match(page, /classList\.toggle\('is-present'/);
assert.match(page, /replace\(\/--\.\*\$\/gm, ' '\)/);
assert.match(page, /data-clause="WHERE"/);
assert.match(page, /data-clause="GROUP BY"/);
