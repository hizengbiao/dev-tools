...
assert.match(page, /id="copyContentBtn"[^]*?data-clipboard-paste-target="contentInput"/);
assert.match(page, /id="copyKeywordsBtn"[^]*?data-clipboard-paste-target="keywordInput"/);
assert.match(page, /ClipboardUtils\.copyText\(document\.getElementById\(inputId\)\.value/);
