...
assert.match(page, /<div class="changelog-date">2026年7月14日<\/div>[\s\S]*?<div class="changelog-version">V1\.01<\/div>[\s\S]*?<div class="changelog-date">2026年7月11日<\/div>[\s\S]*?<div class="changelog-version">V1\.00<\/div>/);
assert.match(page, /\.form-grid label\s*\{/);
assert.match(page, /\.form-grid input,\s*\.form-grid select\s*\{/);
assert.match(page, /\.form-grid input:focus,\s*\.form-grid select:focus\s*\{/);
...
assert.match(page, /function createResultItem\(value, index\)/);
assert.match(page, /valueElement\.textContent = value/);
assert.match(page, /resultList\.replaceChildren\(\.\.\.results\.map\(createResultItem\)\)/);
assert.doesNotMatch(page, /resultList\.innerHTML = results\.map/);
assert.match(page, /\.copy-item-btn\s*\{[^}]*justify-self:\s*end/);
